...
[![Build & publish images](https://github.com/Faris1015/clinical-trial-protocol-screener/actions/workflows/cd.yml/badge.svg?branch=main)](https://github.com/Faris1015/clinical-trial-protocol-screener/actions/workflows/cd.yml)

**Live demo:** deploy your own free, zero-cost copy in minutes —
see [`docs/free-demo-deploy.md`](docs/free-demo-deploy.md). <!-- Add the URL here once deployed. -->
...
pushes the backend + frontend images to GHCR, each tagged with the commit SHA
and `latest`. The backend image bakes in the commit SHA so [`/health`](#health--readiness)
and `/ready` report exactly which build is running.

Deployment itself is handled by the hosting platform's own auto-deploy from the
repo — no deploy step runs in CI. The simplest path is the **free, single-service
demo** (one container serving the SPA + API in stub-LLM mode, no credit card):
see [`docs/free-demo-deploy.md`](docs/free-demo-deploy.md).
